...
This BEP aims to improve the reliability of the drivers, and does **not** address how drivers should implement a [light client protocol][tm:light-client].
...
- `E`, the list of BigchainDB endpoints passed at initialization time (we use the Python notation `len(L)` to get the length of the list).
- `i`, the index of the BigchainDB node to use for a request (`0 ≤ i < len(L)`).
- `T`, a list of timestamps. It has the same length of `L`. On initialization, all elements contain `0`. It is used to track the availability of the nodes. We will see in a second how it is used.
- `DELAY`, in case a node is unreachable, how much milliseconds we should wait. This is used in combination with `T`.
...
1. if `current_time_ms > T[i]`:
    1. forward the request to the node `N[i]`
    1. if the request failed, we need to avoid hitting the unhealthy node for some time `T[i] = current_time_ms + DELAY`
    1. move the index to the next node `i = (i + 1) mod len(N)`
    1. return the result or the error
1. if `current_time_ms ≤ T[i]`:
    1. move the index to the next node `i = (i + 1) mod len(N)`
    1. return error
